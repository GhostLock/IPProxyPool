...
async def validator(queue1, queue2, myip):
    ...
    while True:
        if queue1.empty():
            await asyncio.sleep(config.CHECK_WATI_TIME)
            continue
        proxy = await queue1.get()
        asyncio.ensure_future(detect_proxy(myip, proxy, queue2))    #todo 这里如果采用接收返回值的形式，while True:循环会被阻塞吗？
...
async def detect_proxy(selfip, proxy, queue2=None):
    ...
    protocol, types, speed = await getattr(sys.modules[__name__], config.CHECK_PROXY['function'])(selfip,
